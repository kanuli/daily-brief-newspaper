(() => {
  ...
  const COSY_BACKENDS = [
    {
      ...
      label: "CosyVoice 官方 ZeroGPU",
      schema: "current",
      timeoutMs: 180000
    },
    {
      id: "multimodalart/Fun-CosyVoice3-0.5B-zerogpu",
      label: "CosyVoice HF Staff ZeroGPU",
      schema: "legacy",
      timeoutMs: 180000
    }
  ];
  ...
  function isExcludedParagraph(el) {
    return el.matches?.(".why-mini,.notice,.story-meta,.source-link") || Boolean(el.closest?.(".why-box,.study-desk,[data-no-tts]"));
  }

  function articleSegments(article) {
    const clone = article.cloneNode(true);
    $$("button,a,figure,figcaption,.site-tts-controls,.story-meta,.tag,.eyebrow,.live-badge,.live-time,.source-link,.why-box,.why-mini", clone).forEach((el) => el.remove());

    const titleEl = $("h1,h2,h3", clone);
    const title = textOf(titleEl);

    const explicitSubtitle = $(".lead-deck,.dek,.story-dek,.live-dek,.subtitle,.sub-title", clone);
    let subtitleEl = explicitSubtitle;
    if (!subtitleEl) {
      subtitleEl = $$("p", clone).find((p) => !isExcludedParagraph(p) && !p.closest(".story-body,.article-body")) || null;
    }
    const subtitle = textOf(subtitleEl);

    const bodyCandidates = $$(".story-body p,.article-body p,.live-story > p,.story-card > p,.top-card p,p", clone)
      .filter((p) => p !== subtitleEl && !isExcludedParagraph(p));

    const seen = new Set([title, subtitle].filter(Boolean));
    const bodyParts = [];
    for (const p of bodyCandidates) {
      const text = textOf(p);
      if (!text || seen.has(text)) continue;
      seen.add(text);
      bodyParts.push(text);
      if (bodyParts.join(" ").length >= 620) break;
    }

    const body = cleanText(bodyParts.join(" ")).slice(0, 650);
    const segments = [];
    if (title) segments.push({ kind: "title", text: title });
    if (subtitle && subtitle !== title) segments.push({ kind: "subtitle", text: subtitle });
    if (body && body !== subtitle && body !== title) segments.push({ kind: "body", text: body });

    if (!segments.length) {
      const fallback = cleanText(clone.textContent).slice(0, 900);
      if (fallback) segments.push({ kind: "body", text: fallback });
    }
    return segments;
  }
  ...
  function cosyText(segments) {
    return segments.map((segment) => punctuate(segment.text)).filter(Boolean).join("\n\n").slice(0, MAX_COSY_CHARS);
  }
  ...
  function addButton(article) {
    ...
    if (articleSegments(article).reduce((sum, part) => sum + part.text.length, 0) < 24) return;
  ...
  async function buildReference() {
    ...
    try {
      ...
      return new Blob([encodeWav(out, TARGET_RATE)], { type: "audio/wav" });
    } finally {
  ...
  function backendPayload(backend, text, reference, handle_file) {
    if (backend.schema === "legacy") {
      return {
        tts_text: text,
        mode_checkbox_group: "自然语言控制",
        prompt_text: "",
        prompt_wav_upload: handle_file(reference),
        prompt_wav_record: null,
        instruct_text: CANTONESE_INSTRUCT,
        seed: 42,
        stream: false
      };
    }
  ...
  async function runCosyBackend(backend, text, reference, seq) {
    ...
    const app = await Client.connect(backend.id, {
      events: ["data", "status"],
      space_status: (status) => { spaceState = status?.status || spaceState; }
    });
    if (seq !== activeSequence) throw new Error("cancelled");
    if (spaceState === "space_error" || spaceState === "error" || spaceState === "stopped") throw new Error(`${backend.label} unavailable`);

    const submission = app.submit("/generate_audio", backendPayload(backend, text, reference, handle_file));
    ...
    try {
      for await (const msg of submission) {
        ...
        if (msg.type === "status") {
          if (msg.stage === "error") throw new Error(msg.message || `${backend.label} queue error`);
          if (msg.stage === "pending") {
            const queueText = Number.isFinite(msg.position) ? `（排隊第 ${msg.position + 1}）` : "";
            const etaText = Number.isFinite(msg.eta) ? `，約 ${Math.max(1, Math.round(msg.eta))} 秒` : "";
            setStatus(`${backend.label} 正在等候 GPU ${queueText}${etaText}…`);
          } else if (msg.stage === "generating") {
            setStatus(`${backend.label} 正在生成 F01 女聲廣東話…`);
          }
        }
        if (msg.type === "data") {
    ...
    if (timedOut) throw new Error(`${backend.label} queue timeout`);
    if (!resultUrl) throw new Error(`${backend.label} returned no audio`);
    return resultUrl;
  }
  ...
  async function generateCosy(segments, seq) {
    ...
    for (let i = 0; i < COSY_BACKENDS.length; i += 1) {
      const backend = COSY_BACKENDS[i];
      try {
        return await runCosyBackend(backend, text, reference, seq);
      } catch (error) {
        if (seq !== activeSequence || error?.message === "cancelled") throw error;
        lastError = error;
        console.warn(`${backend.label} failed`, error);
        if (i + 1 < COSY_BACKENDS.length) setStatus(`${backend.label} 未能完成，轉用下一個 CosyVoice ZeroGPU…`);
      }
    }
    throw lastError || new Error("CosyVoice unavailable");
  }
  ...
  async function speakArticle(article, button) {
    ...
    try {
      const url = await generateCosy(segments, seq);
      ...
      try {
        await audio.play();
        setStatus("使用中：CosyVoice · F01 Female（標題／副標題已加入自然停頓）");
      } catch (_) {
        setStatus("CosyVoice · F01 Female 已生成；請按播放器開始。");
      }
    } catch (error) {
      if (seq !== activeSequence) return;
      console.warn("All CosyVoice backends failed", error);
      setStatus("兩個 CosyVoice ZeroGPU 均未能完成，依次嘗試 Google Female → Microsoft Female…");
