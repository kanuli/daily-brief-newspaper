(() => {
  ...
  function render() {
    ...
    if (!correctEngine) { state = "failed"; label = "Production manifest is not Canto Nano verified female"; }
    else if (s.pending === 0 && s.total > 0) { state = "complete"; label = "Current-news verified female coverage complete"; }
    else if (wf.state === "idle") { state = "queued"; label = "Current backlog remains; scheduled Canto Nano continuation enabled"; }
    ...
    row.querySelector(".voice-progress-percent").textContent = `${s.percent.toFixed(1)}%`;
    row.querySelector(".voice-progress-track")?.setAttribute("aria-valuenow", s.percent.toFixed(1));
    row.querySelector(".voice-progress-fill").style.width = `${s.percent.toFixed(2)}%`;
    row.querySelector(".voice-done").textContent = `Done ${s.done}/${s.total}`;
    const creating = state === "active" ? Math.min(MAX_PARALLEL, s.pending) : 0;
    row.querySelector(".voice-creating").textContent = `Creating ${creating}/${s.pending} pending`;
  ...
  async function loadManifest() {
    try { latestManifest = await fetchJson(MANIFEST_PATH); } catch (e) { console.warn("Canto Nano manifest status unavailable", e); latestManifest = null; }
    render();
  }
  async function loadWorkflow() {
    try { const data = await fetchJson(WORKFLOW_API); latestRuns = Array.isArray(data.workflow_runs) ? data.workflow_runs : []; }
    catch (e) { console.warn("Canto Nano workflow status unavailable", e); latestRuns = []; }
    render();
  }
