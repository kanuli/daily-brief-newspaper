(() => {
  ...
  function mountSystemPanel() {
    ...
    panel.innerHTML = `
      ...
      <div class="system-panel-row status-ok">
        ...
        <div><strong>Website</strong><small>Static safe mode · no background monitoring loop</small></div>
      </div>
      <div class="system-panel-row status-ok">
        <span class="status-dot"></span>
        <div><strong>Daily / Live</strong><small>Daily JSON + Live JSON remain the publishing source</small></div>
      </div>
      <div class="system-panel-row status-check">
        <span class="status-dot"></span>
        <div><strong>GitHub Pages / Discord</strong><small>Deployment and push delivery are checked externally</small></div>
      </div>
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", mountSystemPanel, { once: true });
  } else {
    mountSystemPanel();
  }
})();
