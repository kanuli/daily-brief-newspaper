(() => {
  ...
  const POLICY = "f01-news-anchor-v13-approved-young-female-hk-pacing-hktrad";
  const REF_POLICY = "aslp-approved-databaker-female-synthetic-v1";
  const REF_ASSET = "9f24c7f95a2d040c43ce9fadfa56f6f3.wav";
  const CONDITIONING = "approved-young-female-synthetic-bistream";
  const PACING = "hk-tv-news-semantic-pauses-v2";
  ...
  function validEntry(e) {
    return !!(e?.audio && String(e.audio).includes(NAMESPACE) &&
      ...
      e.languageGate === "residual-latin-zero" &&
      e.segmentPolicy === "single-inference-per-article" &&
      e.pacingPolicy === PACING && e.tempoPolicy === TEMPO &&
      Number(e.referenceStartSeconds) === 0 &&
      Number(e.referenceDurationSeconds) === REF_SECONDS &&
      Number(e.speed) === SPEED && Number(e.segmentCount) === 1);
  }
  ...
  function validManifest(m) {
    return !!(m?.engine === "ASLP-lab/Cosyvoice2-Yue" && m?.language === "yue-HK" &&
      ...
      Number(m?.referenceStartSeconds) === 0 &&
      Number(m?.referenceDurationSeconds) === REF_SECONDS && Number(m?.speed) === SPEED);
  }

  function ensureUi() {
    if (!$("#site-tts-v13-style")) {
      const style = document.createElement("style");
      style.id = "site-tts-v13-style";
    ...
    if (!$("#site-tts-player")) {
      ...
      panel.innerHTML = '<div class="site-tts-player-row"><div class="site-tts-status" id="site-tts-status">CosyVoice2-Yue · 年輕 F01 女聲 · v13</div><button type="button" class="site-tts-stop" id="site-tts-stop">■ 停止</button></div><audio class="site-tts-audio" id="site-tts-audio" controls preload="metadata" playsinline></audio>';
  ...
  function play(entry, button = null) {
    ...
    status("使用中：年輕 F01 女聲 · v13 香港新聞主播節奏");
    audio.play()?.catch?.((error) => { console.warn("F01 v13 playback rejected", error); finish(); });
    return true;
  }
  ...
  function configure(article) {
    ...
    if (validEntry(entry)) {
      ...
      button.textContent = READY;
      button.title = "你原先試聽選中的 CosyVoice2-Yue-Databaker 年輕女聲 · v13 · 原生語速1.0";
      button.onclick = () => play(entry, button);
    } else {
      button.disabled = true;
      button.textContent = PENDING;
      button.title = "只播放以原先試聽選中的年輕女聲合成樣本作conditioning的v13音訊。";
    }
  }
  ...
  async function refreshManifest() {
    try {
      ...
      console.warn("F01 v13 manifest unavailable", error);
      manifest = null;
    }
    refreshButtons();
  }
  ...
  window.SiteTTS = {
    playLeadFromUserGesture() { const entry = leadEntry(); return validEntry(entry) ? play(entry) : false; },
    stop: stopAll,
    isReady() { return true; }
  };
  ...
  function boot() {
    ...
    new MutationObserver((mutations) => {
      if (mutations.some((m) => [...m.addedNodes].some(containsArticle))) queueRefresh();
