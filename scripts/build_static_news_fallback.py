#!/usr/bin/env python3
"""Build same-URL static fallbacks for the active public newspaper pages.

The browser JavaScript remains the enhancement/freshness layer. This script
pre-renders the current verified publication into the existing active HTML so a
    ...
    for story_id in ids[:5]:
        ...
        if story:
            rows.append(
                ...
                + f'<h3>{esc(story.get("title"))}</h3><p>{esc(story.get("summary") or story.get("dek"))}</p>{source_link(story)}</div></article>'
            )
    return "".join(rows)
...
def render_desk_latest(desk: dict) -> str:
    ...
    for slug, title, _ in DESKS:
        ...
        if stories:
            story = stories[0]
            rows.append(
                f'<article class="story-card"><div class="tag">{esc(title)}</div><h3>{esc(story.get("title"))}</h3>'
    ...
    for section in latest.get("sections", []):
        ...
        if stories:
            groups.append(
                ...
                + "".join(render_story_card(s, i == 0 and len(stories) > 3) for i, s in enumerate(stories))
                + '</div></section>'
            )
    return "".join(groups)
...
def render_live_page(live: dict):
    ...
    stories = (
        '<p class="notice">目前沒有新增 Live 卡片；Daily Edition / 各版最新仍然保留。</p>'
        if not items
        else "".join(
            f'<article class="live-card"><div class="tag">{esc(item.get("deskLabel") or item.get("desk"))}</div>'
            f'<h2>{esc(item.get("title"))}</h2><p>{esc(item.get("summary") or item.get("dek"))}</p>{source_link(item)}</article>'
            for item in items
        )
    )
...
def render_stock_story(story: dict, index: int) -> str:
    ...
    return (
        ...
        + '<div class="stock-story-body">'
        + "".join(f'<p>{esc(p)}</p>' for p in paras)
        + '</div><div class="stock-info-grid stock-info-grid-three">'
...
def render_stocks(data: dict):
    ...
    for ticker in order:
        ...
        body = (
            "".join(render_stock_story(s, i) for i, s in enumerate(stories))
            if stories
            else '<div class="stock-empty">暫未有已核實的新稿；本節會保留最近有效新聞並在下一輪繼續檢查。</div>'
        )
        sections.append(
            f'<section class="stock-section" id="stock-{esc(ticker.lower())}"><div class="stock-section-head"><div>'
            f'<div class="stock-symbol">{esc(ticker)}</div><div class="stock-name">{esc(block.get("name"))}</div></div>'
            f'<div class="stock-asset-type">{esc(block.get("assetType") or "SECURITY")}</div></div>{body}</section>'
        )
...
def render_archive(data: dict) -> str:
    return "".join(
        f'<a class="archive-item" href="{esc(item.get("url") or item.get("href") or "#")}"><div class="archive-date">{esc(item.get("shortDate"))}</div><div><div class="archive-title">{esc(item.get("headline"))}</div><div class="archive-topics">{esc(" · ".join(item.get("topics") or []))}</div></div><div>閱讀 →</div></a>'
...
def build_index(latest: dict, live: dict, desk: dict) -> None:
    ...
    text = re.sub(
        r'(<span\s+data-edition-date>).*?(</span>)',
        lambda m: m.group(1) + esc(latest.get("dateLabel") or latest.get("date")) + m.group(2),
        text,
        count=1,
        flags=re.S,
    )
    text = re.sub(
        r'(<span\s+data-edition-number>).*?(</span>)',
        lambda m: m.group(1) + esc(latest.get("editionNumber") or "001") + m.group(2),
        text,
        count=1,
        flags=re.S,
    )
...
def main() -> int:
    ...
    print("STATIC NEWS FALLBACK BUILD OK active_pages=13 retail=retired")
    return 0
