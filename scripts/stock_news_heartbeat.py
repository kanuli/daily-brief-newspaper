...
# A fresh, symbol-specific review may retain a still-current verified item when
# there is no stronger new fact. Keep a hard seven-day ceiling so review/check
# timestamps can never make genuinely stale or weeks-old copy current.
MAX_SUBSTANTIVE_STORY_AGE_HOURS = 168.0
...
def main() -> int:
    ...
    stocks["freshnessContract"] = {
        ...
        "reviewRule": "a fresh review may retain a verified still-current item when no stronger new fact exists, but cannot refresh content beyond the hard substantive-age ceiling",
