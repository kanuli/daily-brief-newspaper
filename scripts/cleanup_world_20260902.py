...
# Rolling reservoir: World is strictly non-Asia.
...
dump(p,d)
# Remove any Asian story from World section references in Daily and topic-more; drop orphan article objects.
for p in [DATA/'latest.json',DATA/f'{DATE}.json',DATA/'topic-more'/f'{DATE}.json']:
    ...
    # latest/dated topFive and lead are also references; preserve them unless they are contaminated World-only selections.
    ...
    dump(p,o)
print('WORLD GEOGRAPHY CLEANUP',before,'->',after)
